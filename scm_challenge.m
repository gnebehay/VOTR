...
%Convert to scm format
region(1:2) = region(1:2) + region(3:4) / 2;
...
for f = 1:num
    ...
    corners = [ 1,-w/2,-h/2; 1,w/2,-h/2; 1,w/2,h/2; 1,-w/2,h/2 ]';
    corners = M * corners;
    
    bb(f,:) = reshape(corners, 1,8);
    
    %%----------------- Update Scheme ----------------%%
    upRate = 5;
    if rem(f, upRate)==0
        [A_neg alpha_qq] = updateDic(double(img), sz, opt, param, num_n, p0, f, abs(alpha_q), abs(alpha_p(:,:,id_max)), (b(id_max)/lambda_thr)/prod(patchnum));
    end
end

%%******************************************* Save and Display Tracking Results *********************************************%%

dlmwrite('output.txt', bb);
exit;
